    ...
    for f in files:
        with f.open(newline="") as fh:
            ...
            for row in reader:
                rows_processed += 1
                for m in METRICS:
                    if m in reader.fieldnames and row.get(m):
                        try:
                            val = float(row[m])
                            data[m].append(val)
                        except ValueError:
                            pass
    return data, rows_processed, files
...
def check_invariants(data):
    ...
    # contact_utilization in [0,1] ideally, allow slight >1 due to modeling
    vals = data.get("contact_utilization", [])
    if vals:
        if any(v < 0 for v in vals):
            notes.append("Invariant FAILED: contact_utilization < 0")
        else:
            notes.append("Invariant OK: contact_utilization >= 0")
        # flag >1
        over = [v for v in vals if v > 1.0]
        if over:
            notes.append(f"Warning: contact_utilization >1 in {len(over)} rows (max {max(over):.3f})")
    else:
...
def summarize(data):
    lines = []
    header = ["Metric", "Count", "Min", "Max", "Mean"]
    lines.append("| " + " | ".join(header) + " |")
    lines.append("| " + " | ".join(["---"] * len(header)) + " |")
    for m in METRICS:
        vals = data.get(m, [])
        if not vals:
            lines.append(f"| {m} | 0 | N/A | N/A | N/A |")
        else:
            cnt = len(vals)
            mn = min(vals)
            mx = max(vals)
            mean = statistics.mean(vals)
            lines.append(f"| {m} | {cnt} | {mn:.6g} | {mx:.6g} | {mean:.6g} |")
    return "\n".join(lines)


def main():
    files = scan_csv_files()
    data, rows, files = read_metrics(files)
    invariants = check_invariants(data)
    summary = summarize(data)

    report = []
    report.append("# Audit Report")
    report.append(f"Scanned directory: {RESULTS_DIR}")
